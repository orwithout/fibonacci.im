<script>
    ...
    function selectFile(file) {
        if (selectedFile === file) {
            selectedFile = null; // 如果文件已选中，再次点击则取消选中
        } else {
            selectedFile = file; // 否则设置为选中
        }
    }
...
<button on:click={() => { toggle(); !isFolder(path) && selectFile(path); }}
    class:expanded={isOpen && isFolder(path)}
    class:file={!isFolder(path)}
    class:selected={selectedFile === path && !isFolder(path)}
